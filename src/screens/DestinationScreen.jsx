      ...
      <div className="flex-1 flex flex-col gap-4 px-5 py-6 overflow-auto">
        ...
        <div className="bg-red-600 text-white rounded-2xl p-5 shadow">
          ...
          <div className="mt-4 pt-4 border-t border-white/30 flex items-end justify-between">
            <div>
              <p className="text-white/70 text-xs uppercase tracking-wide">Departs</p>
              <p className="text-3xl font-bold tabular-nums">
                {departure.time}{delayText}
              </p>
            </div>
            <div className="text-right">
              <p className="text-white/70 text-xs uppercase tracking-wide">In</p>
              <p className="text-2xl font-bold">
                {departure.minutesUntil < 1 ? 'Now' : `${departure.minutesUntil} min`}
              </p>
            </div>
          </div>
          {departure.platform && (
            <p className="text-white/60 text-xs mt-2">Platform {departure.platform}</p>
          )}
        </div>

        {/* Neighborhood info */}
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">{destination.neighborhood}</p>
        ...
        <p className="text-center text-gray-400 text-xs px-4">
          Live departure from Bern Bahnhof · transport.opendata.ch
        </p>
      </div>
